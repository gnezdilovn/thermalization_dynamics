...
# perm_diag(n) returns a list of lists of \sigma^y_i
def perm_diag(n):
...
# H_0(n, omega) returns the initial Hamiltonian of the system \omega / 2 \sum_i sigma^y_i
def H_0(n, omega):
    dim = (2 ** n, 2 ** n)
    f_sum = np.zeros(dim)
    L1 = perm_diag(n)
    for i in range(len(L1)):
        ...
        for j in range(len(v)):
            if v[j] == 'Y':
                ...
                v[j] = one
        f = 0.5 * omega * ft.reduce(np.kron, v) 
        f_sum = f_sum + f
    return f_sum 

# H_SYK2(n, couplings, mu) returns 2d array for the Hamiltonian 
# H = H_0 + ( \sum_{i>j}^n J_{ij} c_i^\dag c_j + h.c ) 
# for a given realization of couplings
def H_SYK2(n, couplings, omega):
    dim = (2 ** n, 2 ** n)
    H_sum = np.zeros(dim)
    L1 = perm_SYK2(n)
    Js = couplings
...
# we add the chemical potential        
    H_sum = H_sum + H_0(n, omega)    
    return H_sum  

# evolved_state(n, t, couplings, H) returns the ground state evolved with the Hamiltonian H
...
# p(n, t, states, couplings, H) returns the occupation probability 
...
def SYK2(num, Jc, nr, omega, t_min, t_max, nt):
    ...
    # saving the data:
    np.save('data/SYK2/time.npy', time, allow_pickle = True)
    np.save('data/SYK2/p_N={}_nr={}_omega={}.npy'.format(num, nr, omega), Lp, allow_pickle = True)
    np.save('data/SYK2/pav_N={}_nr={}_omega={}.npy'.format(num, nr, omega), pav, allow_pickle = True)
    np.save('data/SYK2/S_N={}_nr={}_omega={}.npy'.format(num, nr, omega), S, allow_pickle = True)
    np.save('data/SYK2/SS2_N={}_nr={}_omega={}.npy'.format(num, nr, omega), SS2, allow_pickle = True)
    np.save('data/SYK2/Sav_N={}_nr={}_omega={}.npy'.format(num, nr, omega), Sav, allow_pickle = True)
    np.save('data/SYK2/S2av_N={}_nr={}_omega={}.npy'.format(num, nr, omega), S2av, allow_pickle = True)
    np.save('data/SYK2/VarS_N={}_nr={}_omega={}.npy'.format(num, nr, omega), VarS, allow_pickle = True)
    # the data is saved into 'data/SYK2/...'